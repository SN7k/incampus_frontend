...
const LoginForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Basic validation
    if (!universityId) {
      setFormError('University ID or Email is required');
      return;
    }
    ...
    // Check if input is an email or university ID
    const isEmail = universityId.includes('@');
    
    // Only validate university ID format if it's not an email
    if (!isEmail) {
      // Validate university ID format (e.g., BWU/BCA/00/000)
      const idPattern = /^[A-Z]+\/[A-Z]+\/\d+\/\d+$|^[A-Z]+\/[A-Z]+\/[A-Z]+\/\d+$/;
      if (!idPattern.test(universityId)) {
        setFormError('Please enter a valid email address or university ID format (e.g., BWU/BCA/20/123)');
        return;
      }
    }
    
    console.log(`Attempting to login with ${isEmail ? 'email' : 'university ID'}: ${universityId}`);
    ...
    <div className="bg-white p-8 rounded-xl shadow-lg max-w-md w-full">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <LoginInput
          ...
          placeholder="BWU/BCA/00/000 or your@email.com"
          value={universityId}
