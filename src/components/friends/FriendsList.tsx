...
const getMockRequests = (): {id: string, sender: User, createdAt: string}[] => {
  return mockUsers.slice(3, 5).map(user => ({
    id: `request-${user.id}`,
    sender: user,
...
const getMockSuggestions = (): {user: User, mutualFriends: number}[] => {
  return mockUsers.slice(5, 8).map(user => ({
    user,
