...
const Profile: React.FC<ProfileProps> = ({ onNavigate, viewUserId }) => {
  ...
  useEffect(() => {
    // First try to get user data from localStorage if currentUser is null
    let userFromStorage = null;
    if (!currentUser) {
      console.log('Current user is null, trying to recover from localStorage');
      const authState = localStorage.getItem('authState');
      const userData = localStorage.getItem('userData');
      
      if (authState) {
        const parsedAuth = JSON.parse(authState);
        userFromStorage = parsedAuth.user;
        console.log('Recovered user from authState:', userFromStorage);
      } else if (userData) {
        userFromStorage = JSON.parse(userData);
        console.log('Recovered user from userData:', userFromStorage);
      }
    }
    
    if (viewUserId) {
      // First check if the user is in localStorage
      const authState = localStorage.getItem('authState');
      const userData = localStorage.getItem('userData');
      let userFromStorage = null;
      ...
      if (authState) {
        const parsedAuth = JSON.parse(authState);
        if (parsedAuth.user && parsedAuth.user.id === viewUserId) {
          userFromStorage = parsedAuth.user;
        }
      } else if (userData) {
        const parsedUser = JSON.parse(userData);
        if (parsedUser.id === viewUserId) {
          userFromStorage = parsedUser;
        }
      }
      
      if (userFromStorage) {
        setDisplayUser(userFromStorage);
      } else {
        // If not in localStorage, try mockUsers
        const userToShow = mockUsers.find(u => u.id === viewUserId);
        if (userToShow) {
          setDisplayUser(userToShow);
        } else {
          // If user not found, default to current user or user from storage
          setDisplayUser(currentUser || userFromStorage);
        }
      }
    } else {
      // Default to current user if no viewUserId provided
      // If currentUser is null, use the one recovered from localStorage
      setDisplayUser(currentUser || userFromStorage);
    }
  ...
  const handleSaveProfile = async (updatedProfile: { 
    ...
    try {
      ...
      // This will now call the API endpoint to update the database
      await updateUserProfile({
        name: updatedProfile.name,
        bio: updatedProfile.bio,
        avatarFile: updatedProfile.avatar, // Pass the actual file for upload
              ...
              <div className="flex-1">
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-4">
                  <div>
                    ...
                    >
                      {displayUser?.role?.toLowerCase() === 'faculty' ? (
                ...
                >
                  ...
                  <motion.div variants={item} className="bg-gradient-to-br from-purple-50 to-purple-100 px-4 py-3 rounded-xl">
                    ...
                    <div className="text-2xl font-bold text-purple-900">{friends.length}</div>
                  </motion.div>
                  <motion.div variants={item} className="bg-gradient-to-br from-amber-50 to-amber-100 px-4 py-3 rounded-xl">
                    ...
                    <div className="text-2xl font-bold text-amber-900">
                      {displayUser?.batch || (displayUser?.role?.toLowerCase() === 'faculty' ? 'Faculty' : 'Not specified')}
                    </div>
                  </motion.div>
          ...
          {activeTab === 'memories' && (
            <div className="max-w-2xl mx-auto">
              {userPosts.length > 0 ? (
                ...
                <motion.div 
                  className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 sm:p-8 text-center mx-auto max-w-md w-full sm:w-auto md:max-w-lg lg:max-w-xl my-4 sm:my-6"
                  variants={item}
                >
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
                    <BookOpen size={20} className="sm:hidden text-blue-800 dark:text-blue-400" />
                    <BookOpen size={24} className="hidden sm:block text-blue-800 dark:text-blue-400" />
                  </div>
                  <h3 className="text-lg sm:text-xl font-semibold text-gray-800 dark:text-gray-200 mb-2">No memories shared yet</h3>
                  <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400 mb-3 sm:mb-4">Start sharing your university moments with friends!</p>
      ...
      <PostModal
        ...
        onSubmit={async (content, media) => {
          try {
            ...
            await createPost(content, mediaItems);
            
            // Close modal and ensure we're on the memories tab
